...
module X509Certificate2 =
    ...
    let ExportPrivatePem(cert: X509Certificate2) =
        cert.PrivateKey.ExportPkcs8PrivateKey() |> System.Convert.ToBase64String

    /// Export pfx public key as a base64 string
    let ExportPublicPem(cert: X509Certificate2) = cert.Export(X509ContentType.Cert) |> System.Convert.ToBase64String

let printCert (pfxPath: string) (pfxPassphrase: string) =
